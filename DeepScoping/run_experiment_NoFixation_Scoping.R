...
starters<-c(2,3,4)
#Choose between 1--> 5,2 --> 10, 3 --> 15, 4 --> 20
...
library(parallel)
source("R/make_crossing_block_scoping.R")
...
for (change in tp.change) {
  ...
  experiment.sub.results <- mclapply(X = iters.per.core, FUN = function(iter.set) {
    ...
    CAP.genos <- genotype.loci(haploid.genos = CAP.haploids, 
                               genome = hv.genome, 
                               include.QTL = F)
    ...
    parent.selections.i <- select.population(value.mat = predictions.out$GEBV, 
                                             sel.intensity = parents.sel.intensity*scoping, 
                                             selection = "best") 
    ...
    simulation.results[[cycle.name]] <- 
      ...
      for (breeding.cycle in seq(first.bc+1,n.cycles)) {
        ...
        # Make a crossing block
          ...
          crossing.block.i <- make.crossing.block.scoping(Parents = parent.selections.i, 
                                                 Haploids = parent.haploids, 
                                                 N = n.crosses, 
                                                 genome = hv.genome)
        ...
        parent.selections.i <- select.population(value.mat = predictions.out$GEBV, 
                                                 sel.intensity = 100*scoping,#parents.sel.intensity*scoping, 
                                                 selection = "best") 
      ...
      #  topid<-order(parent.selections.i$value.sel, decreasing=T)[1:10]
      #  prebred.names<-parent.selections.i$lines.sel[-topid]
      #  prebred.values<-as.matrix(parent.selections.i$value.sel[-topid,])
        
      #  parent.selections.i$value.sel<-prebred.values
      #  parent.selections.i$lines.sel<-prebred.names
  ...
  filename <- file.path(save.dir, paste("Fixed_Scoping 0.", scoping ,"_",n.iterations,"rep_", change,"_BCstart_",BC.start,"_",date, ".RData", sep = "") )                      
