...
#Version with fixed population
load("Genome/CAP_Genome_QTL100_h0.5_100_Iterations.RData")#------------------------------Set Path To Genome
#Weight variabele to calculate population merrit
c_w<-20
starters<-c(1,2,3,4)
load("Population/Population_DeepScoping.RData")
...
for (change in tp.change) {
  ...
  experiment.sub.results <- mclapply(X = iters.per.core, FUN = function(iter.set) {
    ...
    sapply(X = reps, FUN = function(r) {
      ...
      candidate.genos.use <- Population.list[[rep.iter]]$Population[[BC.start]]$Genotype[,-markers.to.remove]
      ...
      sort.id<-order(value.mat, decreasing=T)
      select.id<-sort.id[1:parents.sel.intensity]
      ...
      M_pop<-Population.list[[rep.iter]]$Population[[BC.start]]$Genotype
      
      f<-matrix(0,dim(M_pop)[2],1)
      rownames(f)<-colnames(M_pop)
      P_rel<-matrix(0,dim(M_pop)[1], dim(M_pop)[2])
      for (i in seq(dim(M_pop)[2])){
        f[i]=(2*sum(M_pop[,i]==1)+sum(M_pop[,i]==0))/(2*dim(M_pop)[1])
        P_rel[,i]<-2*(f[i]-0.5)
      }
      Z<-M_pop-P_rel  
      G<-(Z%*%t(Z))/(2*sum(f*(1-f)))
      theta_w<-mean(G[select.id, select.id]) #averaged relationship
      ...
      while (B_w!=B_w_old){
        ...
        for (i in seq(parents.sel.intensity)){
          for(j in seq(dim(M_pop)[1]-parents.sel.intensity)){
            #substitute selected individual by another available individual
            ...
            theta_w_temp<-mean(G[select.temp, select.temp])
            B_w_temp<-g_w_temp-c_w*theta_w_temp
            
            if (B_w_temp>B_w){
              B_w<-B_w_temp
              theta_w<-theta_w_temp
      ...
      parent.haploids <- select.haploids(haploid.genos = Population.list[[rep.iter]]$Population[[BC.start]]$Haploids,
                                         line.names = parent.selections.i$lines.sel)
      
      parent.values <- select.values(pheno.values.list = Population.list[[rep.iter]]$Population[[BC.start]]$Phenotype, 
                                     line.names = parent.selections.i$lines.sel)
  ...
  filename <- file.path(save.dir, paste("simulation_results_Population_Merit_c_",c_w ,"_rep_", n.iterations,"_", date, ".RData", sep = "") )
