...
#Using a separate population to build a gene bank
#USED TO STUDY THE PRESELECTION at different stages of fixation
#make directory structure
if (file.exists(paste0("Temp"))){
  unlink("Temp", recursive=TRUE)
}
dir.create(paste0("Temp"))
for(i in seq(100)){
  dir.create(paste0("Temp/rep", i))
}

source("R/Prebreeding.R")
source("R/Greedy_Selection_HUC.R")
...
if (experim == 1){
  size.genebank<-500
  greedy_vector<-c(rep(80,100))
  pre_vector<-c(rep(20,100))
  GeneBank.updatePeriod<-1
  starters<-c(1,4)
...
for (change in tp.change) {
      ...
      for (breeding.cycle in seq(first.bc+1,n.cycles)) {
        ...
        simulation.results[[cycle.name]] <- 
          list(geno.summary.stats = list(candidate.af = candidate.af.i,
               ...
               Sel_Var=list( greedy=greedy_vector, pre=pre_vector),
 ...
 # filename <- file.path(save.dir, paste("simulation_results_Gr",greedy, "_Pre",pre,"_Sco",sco,"_BCstart",BC.start,"_update_",GeneBank.updatePeriod,"_" ,date, ".RData", sep = "") )
  filename <- file.path(save.dir, paste("simulation_results_HUC_noUpdate_200QTL_BCstart",BC.start,"_update_",GeneBank.updatePeriod,"_" ,date, ".RData", sep = "") )
